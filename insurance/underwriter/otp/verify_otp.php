...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $conn->prepare("SELECT * FROM underwriter_otp WHERE email = ? AND otp = ? AND expires_at > NOW() AND is_verified = FALSE");
    $stmt->bind_param("ss", $email, $otp);
    $stmt->execute();
    $result = $stmt->get_result();

    if ($result->num_rows > 0) {
        $update_stmt = $conn->prepare("UPDATE underwriter_otp SET is_verified = TRUE WHERE email = ? AND otp = ?");
